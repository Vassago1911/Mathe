...
def generate_all_dags_of_node_count(V:int=7):
    ...
    con.execute( f"""
        CREATE TABLE graphs AS
        SELECT range::BIGINT AS ix 
        FROM range(0, {two_to_the_e}::BIGINT );
        """)
    graphs_cudf = cudf.DataFrame.from_arrow(con.table('graphs').to_arrow_table())
    return graphs_cudf
...
with connect_database() as conn:
    G = generate_all_dags_of_node_count(7) 
    print(G.head())
